...
export async function generateChatFeed({ token, store }: { token: string | null, store: any }) {
    if (!token) return;
    ...
    store.chat.members$.set(prev => mergeById(prev, membersMap));
    store.chat.groups$.set(prev => mergeById(prev, groupsMap));
    store.chat.typing$.set(prev => mergeById(prev, typingMap));
    store.chat.messages$.set(prev => mergeById(prev, messagesMap));
    store.users$.set(prev => ({ ...prev, ...Object.fromEntries(usersMap) }));
    
    store.chat.loaded[token].set(true);
}
